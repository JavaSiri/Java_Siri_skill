---
name: triage
description: 通过分类角色驱动的状态机对数据仓库 issue 进行分类处理。当用户想要创建数据相关 issue、对数据问题进行分类、审核传入的数据质量或 ETL 需求、为 AFK agent 准备数据任务，或管理数据仓库 issue 工作流时使用。
---

# 分类（Triage）

将项目 issue tracker 上的数据仓库 issue 通过一个小型状态机流转到不同的分类角色状态。
...
- `bug` — 数据质量问题、ETL 失败、指标计算错误等
- `enhancement` — 新报表开发、指标新增/变更、数据质量规则补充等

五个**状态**角色：

- `needs-triage` — 需要数据工程师评估
- `needs-info` — 等待报告者或业务方提供更多信息
- `ready-for-agent` — 已完全明确，可交给 AFK agent 处理
- `ready-for-human` — 需要人工实现（手动数据修正、跨团队协调等）
- `wontfix` — 不会处理
...
1. **收集上下文。** 阅读完整的 issue（正文、评论、标签、报告者、日期）。解析之前的分类记录，避免重复询问已解决的问题。探索代码库，使用项目的领域词汇表和数仓分层结构（ODS/DWD/DWS/ADS），遵守所在区域的 ADR。阅读 `.out-of-scope/*.md`，并呈现与该 issue 相似的历史驳回记录。

2. **给出建议。** 告知维护者类别和状态建议及理由，并附上与 issue 相关的数据层和表血缘摘要。等待指示。

3. **验证（数据问题优先）。** 在任何追问之前，尝试数据验证：阅读报告者描述的问题，查询相关表/字段，验证数据质量规则或指标口径是否一致。报告结果 —— 确认问题并定位到具体表/字段是强烈的 `ready-for-agent` 信号；数据不足或口径不明是强烈的 `needs-info` 信号。

4. **追问（如需要）。** 如果 issue 需要补充细节，执行 `/grill-with-docs` 会话。

5. **应用结果：**
   - `ready-for-agent` — 发布一条 agent 简报评论（参见 [AGENT-BRIEF.md](AGENT-BRIEF.md)），明确涉及的数据层、分区、指标口径。
   - `ready-for-human` — 使用与 agent 简报相同的结构，但注明为什么不能委托（需要判断、外部访问、设计决策、手动数据修正）。
   - `needs-info` — 发布分类记录（模板见下方），明确需要补充的数据信息。
...
将在追问中确认的所有内容记录在"目前已确认的内容"下，以免工作丢失。问题必须具体且可操作，例如明确指出期望的指标口径，而不是笼统的"请提供更多信息"。
