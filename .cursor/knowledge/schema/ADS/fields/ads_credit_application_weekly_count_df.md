...
| 属性 | 值 |
|------|-----|
| **表名** | `ttsp_it.ads_credit_application_weekly_count_df` |
...
周维表字段与日维表 `ads_credit_application_daily_count_di` 结构一致，共 72 个字段，区别在于时间维度和环比类型。
...
| 序号 | 字段名 | 数据类型 | 描述 |
|:----:|--------|---------|------|
| — | credit_pass_amount_wow | DECIMAL | 授信通过金额环比增长率 = (当周 - 上周) / 上周 |
| — | credit_pass_amount_wow_delta | DECIMAL | 授信通过金额环比变化量（元） |
| — | credit_pass_num_wow | DECIMAL | 授信通过笔数环比增长率 |
| — | credit_pass_num_wow_delta | DECIMAL | 授信通过笔数环比变化量 |

---

## 关键口径说明

### 与 ads_credit_application_daily_count_di / monthly 的关系

三表字段结构一致，区别仅在时间维度：

| 维度 | daily | weekly | monthly |
|------|-------|--------|---------|
| 时间维度 | credit_application_date | week_start/end_date | month_start/end_date |
| 环比类型 | DoD（日环比） | WoW（周环比） | MoM（月环比） |
| 回刷窗口 | [T-2, T] | [week_start(T-2), T] | [month_start(T-2), T] |
| 写入分区 | ds=credit_application_date | ds=T | ds=T |
