...
| 属性 | 值 |
|------|-----|
| **表名** | `ttsp_it.ads_customer_by_level_daily_di` |
...
日维表字段与月维表 `ads_customer_by_level_monthly_df` 结构一致，共 74 个字段（含 ds）。
...
通过 `cte1_credit_info` 和 `cte3_loan_info` 分别构建授信/借款全局汇总（不区分客户等级），再与客户等级维度数据 LEFT JOIN，计算各类全局占比和通过率。
...
### 与 ads_customer_by_annual_rate_daily_di 的关系
