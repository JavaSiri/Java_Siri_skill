...
| 属性 | 值 |
|------|-----|
| **表名** | `ttsp_it.ads_decision_rule_loan_monthly_count_df` |
...
| 序号 | 字段名 | 数据类型 | 描述 |
|:----:|--------|---------|------|
| 8 | decision_application_num | BIGINT | 当月决策申请累计笔数 |
...
### 与 ads_decision_rule_loan_daily_count_di 的关系

本表为月维汇总表，数据来源于日维表按月聚合：

- 历史月数据从 ds=T-1 分区继承（month_start_date < win_start_ds 的月份）
- 窗口内月份由 ads_node_loan_daily_count_di + dws_decision_rule_loan_daily_count_di 按月重算

### 与 ads_decision_rule_credit_monthly_count_df 的关系
